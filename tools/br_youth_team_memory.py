...
# --------------------------------------------------------------------
#
#  TOOLS FOR YOUTH TEAM MEMORY MANAGEMENT IN BLACKOUT RUGBY AGENT
#
# --------------------------------------------------------------------
# br_team_memory_tools.py
...
SNAPSHOT_DIR = "./memory/youth_team_snapshots"
os.makedirs(SNAPSHOT_DIR, exist_ok=True)

class SaveYouthTeamSnapshot(Tool):
    name = "save_youth_team_snapshot"
    description = (
        "Analytical tool: saves the latest snapshot of a youth team's player data to disk. "
        "This allows tracking changes over time, including all player stats and skills."
    )
    inputs = {
        "team_id": {"type": "integer", "description": "The ID of the youth team to save snapshot for"},
    ...
    def forward(self, team_id: int, players_data: list[dict]) -> bool:
        filename = os.path.join(SNAPSHOT_DIR, f"youth_team_{team_id}.json")
        try:
            with open(filename, "w", encoding="utf-8") as f:
                json.dump(players_data, f, ensure_ascii=False, indent=2)
            return True
        except Exception as e:
            print(f"Error saving snapshot for youth team {team_id}: {e}")
            return False


class LoadYouthTeamSnapshot(Tool):
    name = "load_youth_team_snapshot"
    description = (
        "Analytical tool: loads the last saved snapshot of a youth team's player data from disk. "
        "Returns None if no snapshot exists."
    )
    inputs = {
        "team_id": {"type": "integer", "description": "The ID of the youth team to load snapshot for"}
    }
    output_type = "object"
...
class CompareYouthTeamSnapshots(Tool):
    name = "compare_youth_team_snapshots"
    description = (
        "Analytical tool: compares a new snapshot of a youth team's player data with the previous snapshot. "
    ...
    def forward(self, team_id: int, new_snapshot: list[dict]) -> Optional[dict]:
        old_snapshot = LoadYouthTeamSnapshot().forward(team_id)
        ...
        for name in new_players_by_name:
            if name in old_players_by_name:
                ...
                for attr in ["age", "form", "aggression", "discipline",
                             "leadership", "experience", "weight", "height", "energy"]:
...
class ReportYouthTeamChanges(Tool):
    name = "report_youth_team_changes"
    description = (
        "Analytical tool: converts the output of CompareYouthTeamSnapshots into a readable text report. "
        "Summarizes all changes in player roster and attributes."
    )
    inputs = {
        "changes": {"type": "object", "description": "The dictionary output from CompareYouthTeamSnapshots"}
    }
    output_type = "string"
